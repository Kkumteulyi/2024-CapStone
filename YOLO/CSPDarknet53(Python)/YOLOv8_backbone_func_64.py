...
# 0번 레이어: Conv
input = np.random.randn(1, 3, 64, 64)  # (batch_size, channels, height, width)
...
# 9번 레이어: SPPF
conv1_filters = np.random.randn(64, 64, 3, 3)  # 입력 채널: 256, 출력 채널: 256
conv1_bias = np.random.randn(64)
conv2_filters = np.random.randn(64, 256, 3, 3)  # 입력 채널: 1024, 출력 채널: 256 (concat 후 채널 수)
conv2_bias = np.random.randn(64)

output_9 = sppf_layer(output_4, conv1_filters, conv1_bias, conv2_filters, conv2_bias)
